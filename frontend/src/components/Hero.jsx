...
import { Search, ArrowRight, Play, ChevronDown, Star, LogIn, BotMessageSquare } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../hooks/useAuth';
import ChatBot from '../components/ChatBot'; // Import your chatbot component

const Hero = ({onPostClick, searchValue, setSearchValue, showSearchInNav }) => {
  ...
  const PLACEHOLDER_DURATION = 8000;
  const navigate = useNavigate();
  const { isLoggedIn, authLoading } = useAuth();

  // Predefined placeholder suggestions
  const placeholderSuggestions = [
  "Why is Sigiriya Rock Fortress so famous?",
  "Show me posts about Ella",
  "When is the best time to visit Mirissa?",
  "Which are the most popular temples to visit in Sri Lanka?",
  "How can I plan a 2-day trip to Galle?",
  "Find me posts about wildlife experiences in Yala",
  "What adventure activities are there in Nuwara Eliya?"
  ];
  ...
  const currentPlaceholder = placeholderSuggestions[currentPlaceholderIndex];

  // Handle explore button click
  const handleExploreClick = () => {
    const prompt = searchValue.trim() || currentPlaceholder;
    setChatInitialPrompt(prompt);
    setShowChatBot(true);
  };

  // Handle post selection from chatbot
  const handlePostSelect = (post) => {
    // You can navigate to the post detail or handle it as needed
    console.log('Selected post:', post);
    // Example: navigate(`/post/${post.id}`);
  };
                ...
                >
                  ...
                  >
                    ...
                    <motion.input
                      ...
                      className="w-full pl-12 pr-14 py-4 rounded-full bg-white/0 border border-white/20 backdrop-blur-sm focus:outline-none focus:ring-1.5 focus:ring-white focus:bg-white/20 shadow-2xl text-sm text-white"
                      transition={{ duration: 0.2 }}
                    />
                    
                    {/* Dynamic Animated Placeholder */}
                    <AnimatePresence mode="wait">
                      {!searchValue && (
                        <motion.div
